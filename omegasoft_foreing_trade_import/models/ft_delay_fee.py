...
class FtDelayFee(models.Model):
	_name = 'ft.delay.fee'
	_description = 'Delay fee'
	_order = "tier, id"
	...
	def create(self, value_list):
		res = super(FtDelayFee, self).create(value_list)
		for record in res:
			if record.shipping_company_id:
				record.tier = len(record.shipping_company_id.delay_fee_ids.filtered(lambda line: line.tier != 0)) + 1
			elif record.storage_id:
				record.tier = len(record.storage_id.delay_fee_ids.filtered(lambda line: line.tier != 0)) + 1
		return res

	def unlink(self):
		shipping_company_ids = self.shipping_company_id
		storage_ids = self.storage_id
		res = super(FtDelayFee, self).unlink()
		sequence = 1
		for record in shipping_company_ids.delay_fee_ids:
			record.tier = sequence
			sequence += 1
		for record in storage_ids.delay_fee_ids:
			record.tier = sequence
			sequence += 1
		return res